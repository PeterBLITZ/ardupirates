...
static long movAvgIntZ (long input) {
...
{
  //float tempPresAlt;
  float tempPresAltFiltered;
  
  long absPress =  movAvgIntZ(APM_BMP085.Press);
   
  //tempPresAlt = float(APM_BMP085.Press)/101325.0;
  tempPresAltFiltered = float(absPress)/101325.0;

  //tempPresAlt = pow(tempPresAlt, 0.190295);
  tempPresAltFiltered = pow(tempPresAltFiltered, 0.190295);
 ...
 //   press_baro_altitude = press_baro_altitude * 0.75 + ((1.0 - tempPresAlt) * 4433000)*0.25;  // Altitude in cm (filtered)
	...
	press_baro_altitude  = (1.0 - tempPresAltFiltered) * 4433000;
	
	#if DEBUG_BARO
		 SerPri(press_baro_altitude);
		 comma();
		 SerPrln(press_baro_altitude_filtered);
	#endif
}
#endif
