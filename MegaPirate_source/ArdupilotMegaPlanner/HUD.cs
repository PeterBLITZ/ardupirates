...
namespace hud
{
    public partial class HUD : UserControl
    {
        ...
        [System.ComponentModel.Browsable(true), System.ComponentModel.Category("Values")]
        public float distscale { get; set; }
...
System.ComponentModel.Category("Values")]
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            try
            {
                ...
                for (int a = -90; a <= 90; a += 5)
                {
                    ...
                    //if (a >= pitch - 20 && a <= pitch + 20)
                    {
                        if (a % 10 == 0)
                        {
                            ...
                            drawstring(graphicsObject, a.ToString(), new Font("Arial", fontsize + 2), whiteBrush, this.Width / 2 - lengthlong - 30 - halfwidth - (int)(fontoffset * 1.7), pitchoffset + a * every5deg - 8 - fontoffset);
                        }
                        else
                        {
                ...
                for (int a = -45; a <= 45; a += 15)
                {
                    ...
                    drawstring(graphicsObject, Math.Abs(a).ToString("##"), new Font("Arial", fontsize), whiteBrush, 0 - 6 - fontoffset, -lengthlong * 2 - extra);
                ...
                for (int a = start; a <= heading + 30; a += 1)
                {
                    ...
                    if (a % 5 == 0)
                    {
                        ...
                        if (disp == 0)
                        {
                            drawstring(graphicsObject, "N".PadLeft(2), new Font("Arial", fontsize + 4), whiteBrush, headbg.Left - 5 + space * (a - start) - fontoffset, headbg.Bottom - 24 - (int)(fontoffset * 1.7));
                        }
                        else if (disp == 90)
                        {
                            drawstring(graphicsObject, "E".PadLeft(2), new Font("Arial", fontsize + 4), whiteBrush, headbg.Left - 5 + space * (a - start) - fontoffset, headbg.Bottom - 24 - (int)(fontoffset * 1.7));
                        }
                        else if (disp == 180)
                        {
                            drawstring(graphicsObject, "S".PadLeft(2), new Font("Arial", fontsize + 4), whiteBrush, headbg.Left - 5 + space * (a - start) - fontoffset, headbg.Bottom - 24 - (int)(fontoffset * 1.7));
                        }
                        else if (disp == 270)
                        {
                            drawstring(graphicsObject, "W".PadLeft(2), new Font("Arial", fontsize + 4), whiteBrush, headbg.Left - 5 + space * (a - start) - fontoffset, headbg.Bottom - 24 - (int)(fontoffset * 1.7));
                        }
                        else
                        {
                            drawstring(graphicsObject, (disp % 360).ToString().PadLeft(3), new Font("Arial", fontsize), whiteBrush, headbg.Left - 5 + space * (a - start) - fontoffset, headbg.Bottom - 24 - (int)(fontoffset * 1.7));
                        }
                    }
                }
                ...
                for (int a = start; a <= (speed + viewrange / 2); a += 1)
                {
                    ...
                    if (a % 5 == 0)
                    {
                        ...
                        drawstring(graphicsObject, a.ToString().PadLeft(5), new Font("Arial", fontsize), whiteBrush, scrollbg.Right - 50 - 4 * fontoffset, scrollbg.Top - space * (a - start) - 6 - fontoffset);
                    }
                }

                graphicsObject.DrawPolygon(blackPen, arrow);
                graphicsObject.FillPolygon(Brushes.Black, arrow);
                drawstring(graphicsObject, ((int)speed).ToString("0"), new Font("Arial", 10), Brushes.AliceBlue, 0, -9);

                graphicsObject.ResetTransform();

                // extra text data

                drawstring(graphicsObject, "AS " + airspeed.ToString("0.0"), new Font("Arial", fontsize), whiteBrush, 1, scrollbg.Bottom + 5);
                drawstring(graphicsObject, "GS " + groundspeed.ToString("0.0"), new Font("Arial", fontsize), whiteBrush, 1, scrollbg.Bottom + fontsize + 2 + 10);
                ...
                for (int a = start; a <= (alt + viewrange / 2); a += 1)
                {
                    ...
                    if (a % 5 == 0)
                    {
                        ...
                        drawstring(graphicsObject, a.ToString().PadLeft(5), new Font("Arial", fontsize), whiteBrush, scrollbg.Left + 7 + (int)(0 * fontoffset), scrollbg.Top - space * (a - start) - 6 - fontoffset);
                    }
                }
                ...
                drawstring(graphicsObject, ((int)alt).ToString("0"), new Font("Arial", 10), Brushes.AliceBlue, scrollbg.Left + 10, -9);
                graphicsObject.ResetTransform();

                // mode and wp dist and wp

                drawstring(graphicsObject, mode, new Font("Arial", fontsize), whiteBrush, scrollbg.Left - 30, scrollbg.Bottom + 5);
                drawstring(graphicsObject, (int)disttowp + ">" + wpno, new Font("Arial", fontsize), whiteBrush, scrollbg.Left - 30, scrollbg.Bottom + fontsize + 2 + 10);

                // battery

                graphicsObject.ResetTransform();

                drawstring(graphicsObject, resources.GetString("Bat"), new Font("Arial", fontsize + 2), whiteBrush, fontsize, this.Height - 30 - fontoffset);
                drawstring(graphicsObject, batterylevel.ToString("0.00v"), new Font("Arial", fontsize + 2), whiteBrush, fontsize * 4, this.Height - 30 - fontoffset);
                drawstring(graphicsObject, batteryremaining.ToString("0%"), new Font("Arial", fontsize + 2), whiteBrush, fontsize * 9, this.Height - 30 - fontoffset);
                ...
                drawstring(graphicsObject, gps, new Font("Arial", fontsize + 2), whiteBrush, this.Width - 10 * fontsize, this.Height - 30 - fontoffset);
        ...
        float wrap360(float noin)
    {
        ...
        void drawstring(Graphics e, string text, Font font, Brush brush, float x, float y)
        {
