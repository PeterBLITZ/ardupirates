...
void readSerialCommand() {
  ...
  if (SerAva()) {
    ...
    /*case 'O': // Rate Control PID
      ...
      break;*/
      case 'O': //motor offset commands
      pitch_motor_offset=readFloatSerial();
      roll_motor_offset=readFloatSerial();
...
//    SerPri("command rx yaw =");
//    SerPriln(command_rx_yaw);
    SerPri("Motor Offsets-pitch, roll:");
    SerPri(pitch_motor_offset);
    comma();
    SerPri(roll_motor_offset);
    SerPriln(); 
    queryType = 'X';
     break;
  case 'U': // Send receiver values
    ...
    break;
  case 'V': // Spare (GPS lat/long for now)
    SerPri(GPS.Lattitude);
    comma();
    SerPri(GPS.Longitude);
    comma();
    SerPri(target_lattitude - GPS.Lattitude);
    comma();
    SerPri(target_longitude - GPS.Longitude);
    comma();
    SerPri(command_gps_pitch);
    comma();
    SerPri(command_gps_roll);
    comma();
    //SerPri(GPS.Ground_Course/100.0,DEC);
    SerPri(Waypoint_counter);
    comma();
    SerPriln(ToDeg(APM_Compass.Heading));
    break;
