...
//        error 		= constrain(error, -2500, 2500);
	error 		= constrain(error, -5000, 5000);
...
//	g.rc_1.servo_out	= max(g.rc_1.servo_out, -2500);
	g.rc_1.servo_out	= min(g.rc_1.servo_out, 5000);
	g.rc_1.servo_out	= max(g.rc_1.servo_out, -5000);
}
...
//	g.rc_2.servo_out	= max(g.rc_2.servo_out, -2500);
	g.rc_2.servo_out	= min(g.rc_2.servo_out, 5000);
	g.rc_2.servo_out	= max(g.rc_2.servo_out, -5000);
}
...
//	nav_throttle = g.throttle_cruise + constrain(nav_throttle, -60, 60);
	nav_throttle = g.throttle_cruise + constrain(nav_throttle, -300, 300);
