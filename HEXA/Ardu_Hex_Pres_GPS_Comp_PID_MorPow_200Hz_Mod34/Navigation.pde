...
// Altitude control...Hein's Version 
void BMP_Altitude_control(float BMP_target_alt)
{
  ...
  if (BMP_command_altitude < -30)    // Limit the copter from falling to fast out of the sky.  
    BMP_command_altitude = -30;      //Somehow the constrain function -30,50 cause the heading hold to drift.
}
