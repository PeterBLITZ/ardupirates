...
#define THROTTLE_YAW_EFFECT_ADDR EEPROM_BASE_ADDRESS+44
#define ROLL_I_VALUE_ADDR EEPROM_BASE_ADDRESS+48
#define PITCH_I_VALUE_ADDR EEPROM_BASE_ADDRESS+52
#define HEADING_I_VALUE_ADDR EEPROM_BASE_ADDRESS+56
#define YAW_I_VALUE_ADDR EEPROM_BASE_ADDRESS+60
...
#define HELI_STICK_TO_ANGLE_FACTOR 1   // To convert ccpm values (-50 ~ 50 ) to absolute angles.  larger number means less lean
...
#define HELI_YAW_MAX_RATE  400           // max yaw rate (used for heading hold.  max speed in rate mode (i.e. from user moving stick) is controlled above

// movement sensing
#define HELI_GYRO_NO_MOVEMENT_VALUE 1.0
#define HELI_SETTING_SAVE_AFTER_MILLIS_OF_MOVEMENT 30000  // only save navigation values if we've flown for at least 30seconds
#define HELI_SETTING_SAVE_AFTER_MILLIS_OF_STABILITY 5000  // save settings after this many milliseconds have passed without movement
int heli_movement_counter = 0;
int heli_moving = FALSE;
unsigned long heli_startedMovingTime = 0;
unsigned long heli_stoppedTime = 0;
unsigned long heli_saveSettings = FALSE;


// gyro filtering
#define HELI_GYRO_NUM_AVERAGING 3
float heli_roll_filter[HELI_GYRO_NUM_AVERAGING];
float heli_pitch_filter[HELI_GYRO_NUM_AVERAGING];
float heli_yaw_filter[HELI_GYRO_NUM_AVERAGING];
int heli_filter_ptr = 0;
...
//                              0.0000, -1.0000, 1.0000
  #define CCPM_DEALLOCATION   1.0000, -1.0000, 0.0000,  \
                              0.5000, 0.5000, -1.0000,  \
                              0.3333, 0.3333, 0.3333
  #define CCPM_ALLOCATION     0.5000, 0.3333,  1.0000,  \
...
// cross effects
float throttle_yaw_effect = 0.0;  // yaw rate impact of increasing collective pitch by 1%.  We will use this to counteract the effect
