...
//#define ROLL_STICK_TO_ANGLE_FACTOR 1.0        // To convert stick position to absolute angles
//#define PITCH_STICK_TO_ANGLE_FACTOR 1.0        // To convert stick position to absolute angles
...
//  Can be used for PID tuning (see FUNCTIONS) or Camera 3 position tilt (pitch).
    ...
    // We apply the Radio calibration parameters (from configurator) except for throttle
    // left channel
    ccpmValues.x  = frontLeftCCPMslope * APM_RC.InputCh(CHANNEL_FRONT_LEFT) + frontLeftCCPMintercept;
    
    // right channel
    ccpmValues.y = frontRightCCPMslope * APM_RC.InputCh(CHANNEL_FRONT_RIGHT) + frontRightCCPMintercept;
    
    // rear channel
    ccpmValues.z = rearCCPMslope * APM_RC.InputCh(CHANNEL_REAR) + rearCCPMintercept;
    
    // decode the ccpm
    rollPitchCollValues = ccpmDeallocation * ccpmValues;
    ...
    ch_roll = rollPitchCollValues.x;
    ch_pitch = rollPitchCollValues.y;
    ch_collective = rollPitchCollValues.z;
    ...
    command_rx_roll = ch_roll / STICK_TO_ANGLE_FACTOR;        // Convert stick position to absolute angles
    command_rx_pitch = ch_pitch / STICK_TO_ANGLE_FACTOR;      // Convert stick position to absolute angles
