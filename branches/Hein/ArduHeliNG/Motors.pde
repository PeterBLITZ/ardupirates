...
void motor_output()
{
  ...
  if ((throttle_mode==0)&&(ch_throttle < (MIN_THROTTLE + 100)))  // If throttle is low we disable yaw (neccesary to arm/disarm motors safely)
    ...
    pitchRollCollValues_out.x = control_roll;
    pitchRollCollValues_out.y = control_pitch;
    pitchRollCollValues_out.z = throttle;
    
    ccpmValues_out = ccpmAllocation * pitchRollCollValues_out;
    // calculate values to be sent out to RC Output channels

    leftOut =  (ccpmValues_out.x - frontLeftCCPMintercept) / frontLeftCCPMslope;
    rightOut = (ccpmValues_out.y - frontRightCCPMintercept) / frontRightCCPMslope;
    rearOut =  (ccpmValues_out.z - rearCCPMintercept) / rearCCPMslope;
    ...
    APM_RC.OutputCh(CHANNEL_YAW_GAIN, 1300);
