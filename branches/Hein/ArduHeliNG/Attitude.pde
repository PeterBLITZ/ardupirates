...
{
  #define MAX_CONTROL_OUTPUT 600  //250
  float stable_roll,stable_pitch,stable_yaw;
  
  // ROLL CONTROL    
  err_roll = command_roll - ToDeg(roll);
  err_roll = constrain(err_roll,-250,250);  // to limit max roll command...
  
  roll_I += err_roll*G_Dt;
  roll_I = constrain(roll_I,-200,200);
  ...
  err_pitch = constrain(err_pitch,-250,250);  // to limit max pitch command...
  
  pitch_I += err_pitch*G_Dt;
  pitch_I = constrain(pitch_I,-200,200);
