...
#define LOG_BARO_MSG			0x0C
#define LOG_FLIGHT_DATA_MSG		0x0D
#define LOG_GPS_RTL      		0x0E
...
// Write a flight data packet.
#if LOG_FLIGHT_DATA
void Log_Write_Flight_Data(long currentTime, long roll, long pitch, long yaw, int control_roll, int control_pitch, int control_yaw, long heading, long ch_trottle, int frontMotor, int backMotor, int rightMotor, int leftMotor, unsigned long maxloopTime)
{
  DataFlash.WriteByte(HEAD_BYTE1);
  DataFlash.WriteByte(HEAD_BYTE2);
  DataFlash.WriteByte(LOG_FLIGHT_DATA_MSG);
  DataFlash.WriteLong(currentTime); //timer from start in millis
  DataFlash.WriteLong(roll);
  DataFlash.WriteLong(pitch);
  DataFlash.WriteLong(yaw);
  DataFlash.WriteInt(int(control_roll));
  DataFlash.WriteInt(int(control_pitch));
  DataFlash.WriteInt(int(control_yaw)); 
  DataFlash.WriteLong(heading); 
  DataFlash.WriteLong(ch_trottle);
  DataFlash.WriteInt(frontMotor); 
  DataFlash.WriteInt(backMotor);
  DataFlash.WriteInt(rightMotor);
  DataFlash.WriteInt(leftMotor);
  DataFlash.WriteLong(maxLoopTime); //max time to execute the loop in micros()
...
void Log_Write_GPS(	long log_Time, long log_latitude, long log_longitude, long log_gps_alt, 
long log_Ground_Speed, long log_Ground_Course, byte log_Fix, byte log_NumSats, long gps_hdop)
{
  DataFlash.WriteByte(HEAD_BYTE1);
  DataFlash.WriteByte(HEAD_BYTE2);
...
#if LOG_GPS_RTL
// Write an GPS packet. Total length : 30 bytes
void Log_Write_GPS_RTL(	long home_loc_lat, long home_loc_lng, long home_loc_alt, long distance_to_home, long target_bearing)
{
  DataFlash.WriteByte(HEAD_BYTE1);
  DataFlash.WriteByte(HEAD_BYTE2);
  DataFlash.WriteByte(LOG_GPS_RTL);
  DataFlash.WriteLong(home_loc_lat);
  DataFlash.WriteLong(home_loc_lng);
  DataFlash.WriteLong(home_loc_alt);
  DataFlash.WriteLong(distance_to_home);
  DataFlash.WriteLong(target_bearing);
  DataFlash.WriteByte(END_BYTE);
}
#endif
...
#ifdef LOG_BARO
void Log_Write_Baro(long baro_altitude, long press_baro_altitude_filtered, long temp, long target_baro_altitude)
{
  DataFlash.WriteByte(HEAD_BYTE1);
  DataFlash.WriteByte(HEAD_BYTE2);
  DataFlash.WriteByte(LOG_BARO_MSG);
  DataFlash.WriteLong(baro_altitude);
  DataFlash.WriteLong(press_baro_altitude_filtered);
  DataFlash.WriteLong(temp);
  DataFlash.WriteLong(target_baro_altitude);
  DataFlash.WriteByte(END_BYTE);
}
#endif
...
void Log_Read_GPS()
{
  ...
  SerPri(",");
  SerPri(log_hdop/100.0);
  SerPriln();

}

// Read a GPS packet
void Log_Read_GPS_RTL()
{

  long log_latitude;
  long log_longitude;
  long log_gps_alt;
  long log_distance_home;
  long log_target_bearing;

  log_latitude = DataFlash.ReadLong();
  log_longitude = DataFlash.ReadLong();
  log_gps_alt = DataFlash.ReadLong();
  log_distance_home = DataFlash.ReadLong();
  log_target_bearing = DataFlash.ReadLong();
  ...
  SerPri(log_gps_alt);
  SerPri(",");
  SerPri(log_distance_home);			
  SerPri(",");
  SerPri((float)log_target_bearing/100, DEC);
  SerPriln();
...
void Log_Read(int start_page, int end_page)
{
	...
	while (DataFlash.GetPage() < end_page && numErrors < LOG_MAX_ERRORS)
	{
        ...
        else if(data==LOG_GPS_RTL){
        Log_Read_GPS_RTL();
        log_step++;

      }
      else if(data==LOG_FLIGHT_DATA_MSG){
        Log_Read_Flight_Data();
        log_step++;
