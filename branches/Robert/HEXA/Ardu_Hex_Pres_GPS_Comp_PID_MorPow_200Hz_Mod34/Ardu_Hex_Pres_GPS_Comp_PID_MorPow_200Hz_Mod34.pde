...
/*
**** Switch Functions *****
//     We read the Quad Mode from Gear and Aux2 Channel on radio (example Spektrum)
...
#define DECLINATION -15.65      // Hein, South Africa, Centurion. Adjusting offset of Magnetometer.
...
/* Software version */
#define VER 3.3    // Current software version (only numeric values)
...
// Filter of Altitude Hold - Hein's Version 
...
void loop(){
  ...
  if((millis()-timer)>=5)   // Main loop 200Hz
  {
    ...
    if (BMP_counter > 10)  // Reading Barometric data at 20Hz 
    {
      BMP_counter = 0;
      APM_BMP085.Read();
      BMP_Altitude = BMP_filter(APM_BMP085.Press, BMP_Altitude);
    }
#endif
