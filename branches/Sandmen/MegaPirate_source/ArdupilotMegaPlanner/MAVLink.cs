...
namespace ArdupilotMega
{
    public partial class MAVLink : SerialPort
    {
        ...
        public byte[] packetspersecond = new byte[256];
        ...
        public void requestDatastream(byte id,byte hzrate)
        {
            ...
            switch (id) {
                ...
                case (byte)MAVLink.MAV_DATA_STREAM.MAV_DATA_STREAM_RAW_CONTROLLER:
                    ...
                    if (pps >= (hzrate - 2) && pps <= (hzrate + 2) || pps > 30)
                    {
                        return;
                    }
                    break;
        ...
        public byte[] readPacket()
        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    while (this.BytesToRead < length)
                                    {
                                        if (to > 1000)
                                        {
                                            Console.WriteLine("MAVLINK: wait time out {0} {1}", this.BytesToRead, length);
                                            break;
                                        }
                                        System.Threading.Thread.Sleep(1);
                                        to++;
                                    }
                                    int read = this.Read(temp, 2, length);
                                }
            ...
            if (temp.Length >= 5 && sysid != 0)
            {
                if (sysid != temp[3] || compid != temp[4])
                {
                    Console.WriteLine("Mavlink Bad Packet (not addressed to this MAV) {0} {1}", temp[3], temp[4]);
                    return new byte[0];
                }
            }
            ...
            try
            {
                if (temp[0] == 'U' && temp.Length >= temp[1])
                {
                    ...
                    packetspersecond[temp[5]] = (byte)(((1000 / (DateTime.Now - packetspersecondbuild[temp[5]]).TotalMilliseconds) + packetspersecond[temp[5]] ) / 2);
