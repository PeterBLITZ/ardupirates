            ...
            if (fbd.SelectedPath != "")
            {
                ...
                foreach (string file in files)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (entry.Key.ToString().EndsWith(".Text") || entry.Key.ToString().EndsWith("HeaderText") || entry.Key.ToString().EndsWith("ToolTipText"))
                            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                if (row.Cells[colFile.Index].Value.ToString() != fname)
                {
                    if (writer !=null)
                        writer.Close();
                    writer = new ResXResourceWriter(row.Cells[colFile.Index].Value.ToString().Replace(".resx", "." + ci + ".resx"));
                }
