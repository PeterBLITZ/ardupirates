...
namespace ArdupilotMega
{
    static class Program
    {
        ...
        static void Application_ThreadException(object sender, System.Threading.ThreadExceptionEventArgs e)
        {
            ...
            if (e.Exception.GetType() == typeof(FileNotFoundException) || e.Exception.GetType() == typeof(BadImageFormatException)) // i get alow of error from people clikc the exe from inside a zip file.
            {
            ...
            if (DialogResult.Yes == dr)
            {
                try
                {
                    ...
                    string postData = "message=" + Environment.OSVersion.VersionString + " " + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString() + " " + Application.ProductVersion + " Exception " + ex.ToString() + " Stack: " + ex.StackTrace.ToString().Replace('&',' ').Replace('=',' ');
                ...
                catch { MessageBox.Show("Error sending Error report!! You most likerly are not on the internet"); }
            }
        }
    }
}
