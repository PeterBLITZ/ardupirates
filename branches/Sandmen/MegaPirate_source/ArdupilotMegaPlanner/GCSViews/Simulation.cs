...
namespace ArdupilotMega.GCSViews
{
    public partial class Simulation : UserControl
    {
        ...
        private void mainloop()
        {
            ...
            DateTime lastdata = DateTime.Now.AddSeconds(-5);

            while (threadrun == 1)
            {
                if (comPort.IsOpen == false) { break; }
                // re-request servo data
                if (!(lastdata.AddMilliseconds(8000) > DateTime.Now)) // 2000 milliseconds
                {
...
//                Console.WriteLine("extra ax " + ax + " ay " + ay + " az " + az);
        ...
        private void processArduPilot()
        {
            ...
            if (RAD_softXplanes.Checked)
            {
                ...
                a += 4;
                Array.Copy(BitConverter.GetBytes((float)(-999)), 0, Xplane, a, 4); // start data
                a += 4;
                Array.Copy(BitConverter.GetBytes((float)(-999)), 0, Xplane, a, 4); // this one
                a += 4;
