...
namespace ArdupilotMega.GCSViews
{
    partial class FlightPlanner : UserControl
    {
        ...
        public void callMeDrag(string pointno, double lat, double lng, int alt)
        {
            ...
            {
                MessageBox.Show("Apparently row " + selectedrow + " does not exist !! How did you get this error??");
                return;
            }
        ...
        public void callMeDisplay(double lat, double lng, int alt)
        {
            ...
            try
            {
                ...
                lbl_prevdist.Text = (lastdist * 1000).ToString(rm.GetString("lbl_prevdist.Text") + ": 0.00 m");

                double homedist = MainMap.Manager.GetDistance(currentMarker.Position, polygon.Points[0]);

                lbl_homedist.Text = (homedist * 1000).ToString(rm.GetString("lbl_homedist.Text") + ": 0.00 m");
            }
            catch { }
        }
            ...
            try
            {
                ...
                switch (option)
                {
                    ...
                    case "DO_REPEAT_RELAY":
                        Commands.Columns[1].HeaderText = "Relay No";
                        Commands.Columns[2].HeaderText = "Count";
                        Commands.Columns[3].HeaderText = "Cycle (sec)";
                        Commands.Columns[4].HeaderText = "N/A";
                        break;
                    case "DO_SET_RELAY":
                        Commands.Columns[1].HeaderText = "Relay No";
                        Commands.Columns[2].HeaderText = "Off(0)/On(1)";
                        Commands.Columns[3].HeaderText = "N/A";
                        Commands.Columns[4].HeaderText = "N/A";
                        break;
                    case "DO_SET_SERVO":
                        Commands.Columns[1].HeaderText = "Servo No";
                        Commands.Columns[2].HeaderText = "Value";
                        Commands.Columns[3].HeaderText = "N/A";
                        Commands.Columns[4].HeaderText = "N/A";
                        break;
                    case "DO_REPEAT_SERVO":
                        Commands.Columns[1].HeaderText = "Servo No";
                        Commands.Columns[2].HeaderText = "Value";
                        Commands.Columns[3].HeaderText = "Count";
                        Commands.Columns[4].HeaderText = "Delay";
                        break;
                    default:
            ...
            try
            {
                ...
                if (polygon != null && polygon.Points.Count > 0)
                {
                    ...
                    if (home.Length > 5)
                    {
                        ...
                        lbl_homedist.Text = homedist.ToString(rm.GetString("lbl_homedist.Text") + ": 0.00 m");
                    }
                    //lbl_distance.Text = (polygon.Distance + homedist).ToString("Distance: 0.0000 Km");
            ...
            if (file != "")
            {
                try
                {
                    ...
                    for (int a = 0; a < Commands.Rows.Count - 0; a++)
                    {
                        ...
                        sw.Write(double.Parse(Commands.Rows[a].Cells[2].Value.ToString(),new System.Globalization.CultureInfo("en-US")) + ",");
                        sw.Write(double.Parse(Commands.Rows[a].Cells[3].Value.ToString(), new System.Globalization.CultureInfo("en-US")) + ",");
                        sw.Write(double.Parse(Commands.Rows[a].Cells[4].Value.ToString(),new System.Globalization.CultureInfo("en-US")) + "}");
            ...
            System.Threading.Thread t12 = new System.Threading.Thread(delegate()
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        home.alt = (int)(float.Parse(TXT_homealt.Text) * 100); // use saved home
                    }
                    ...
                    for (int a = 0; a < Commands.Rows.Count - 0; a++)
                    {
                        ...
                        if (temp.id < (byte)MAVLink.MAV_CMD.LAST)
                        {
                            ...
                            temp.alt = (int)(double.Parse(Commands.Rows[a].Cells[2].Value.ToString()) * 100);
                    ...
                    {
                        port.setParam("ALT_HOLD_RTL", int.Parse(TXT_DefaultAlt.Text) * 100);
                    }
                    else
                    {
                    ...
                    port.setParam("WP_RADIUS", (byte)int.Parse(TXT_WPRad.Text));

                    try
                    {
                        port.setParam("LOITER_RADIUS", (byte)int.Parse(TXT_loiterrad.Text));
                    }
                    catch
                    {
                        port.setParam("WP_LOITER_RAD", (byte)int.Parse(TXT_loiterrad.Text));
                    }
                ...
                if (cellhome.Value != null)
                {
                    if (cellhome.Value.ToString() != TXT_homelat.Text)
                    {
                        ...
                        if (dr == DialogResult.Yes)
                        {
                            ...
                            TXT_homealt.Text = (double.Parse(cellhome.Value.ToString())).ToString();
                        }
                    }
                }

                TXT_DefaultAlt.Text = ((float)param["ALT_HOLD_RTL"] / 100).ToString();
                TXT_WPRad.Text = ((float)param["WP_RADIUS"]).ToString();
                try
                {
                    TXT_loiterrad.Text = ((float)param["LOITER_RADIUS"]).ToString();
                }
                catch
                {
                    TXT_loiterrad.Text = ((float)param["WP_LOITER_RAD"]).ToString();
                }
        ...
        private void BUT_grid_Click(object sender, EventArgs e)
        {
            ...
            if (!area.IsEmpty)
            {
                ...
                int overshootdist = (int)double.Parse(overshoot);

                int altitude = (int)double.Parse(alt);

                double latdiff = area.HeightLat / ((heightdist / double.Parse(distance)));
