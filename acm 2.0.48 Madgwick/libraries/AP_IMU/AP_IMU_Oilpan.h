...
private:
    ...
    float				_sensor_in(uint8_t channel, uint16_t adc_value);
    float 		        _sensor_compensation(uint8_t channel) const;
	...
	static const float		_gyro_max_cal_offset = 360.0;
	static const float		_accel_total_cal_change = 8.0;
	static const float		_accel_max_cal_offset = 290.0;
