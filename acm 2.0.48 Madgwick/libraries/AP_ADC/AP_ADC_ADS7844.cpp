...
static uint16_t	prevValue[8]	= { 0, 0, 0, 0, 0, 0, 0, 0 };
...
#define TCNT2_2_kHZ		(131)
#define TCNT2_2700_HZ	(167)
...
//	TCCR2B = _BV(CS21) | _BV(CS22);	// Set prescaler of clk/256
	TCCR2B = _BV(CS22);	// Set prescaler of clk/64
	TCNT2	= 0;
...
uint16_t AP_ADC_ADS7844::readPitot()
{
	uint16_t count;
	uint32_t sum;

	// ensure we have at least one value
	while (_count[7] == 0) ; // noop
